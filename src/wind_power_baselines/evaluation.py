...
def evaluate_predictions(predictions: pd.DataFrame) -> pd.DataFrame:
    require_columns(predictions, ["method", "month", "energy_pred", "energy_true"], "prediction data")
    ...
    if data["energy_true"].le(0.0).any():
        raise ValueError("all truth values must be positive")
    ...
    for method, group in data.groupby("method", sort=False):
        ...
        rows.append({"method": str(method), "month_count": int(len(group)), "mape_percent": round(float(group["absolute_percent_error"].mean()), 4), "bias_percent": round((total_pred - total_true) / total_true * 100.0, 4), "pearson_r": pearson_r})

...
def require_columns(data: pd.DataFrame, required_columns: Sequence[str], dataset_name: str) -> None:
    ...
    if missing:
        raise ValueError(f"{dataset_name} 缺少必要字段: {missing}; 实际字段: {list(data.columns)}")


def month_from_timestamp(values: pd.Series) -> pd.Series:
...
def clean_scada(raw_scada: pd.DataFrame) -> pd.DataFrame:
    require_columns(raw_scada, ["timestamp", "turbine_id", "wind_speed", "power_kw", "status"], "SCADA样例数据")
    data = raw_scada.copy()
    ...
    valid = data[
        ...
        & data["wind_speed"].between(0.0, 35.0, inclusive="both")
        & data["power_kw"].between(0.0, 5000.0, inclusive="both")
        & data["status"].str.lower().eq("normal")
    ].copy()
    if valid.empty:
        raise ValueError("SCADA样例数据清洗后为空，请检查时间、风速、功率和状态字段")
...
def prepare_forecast(raw_forecast: pd.DataFrame) -> pd.DataFrame:
    require_columns(raw_forecast, ["issue_time", "target_time", "lead_day", "wind_speed"], "气象预报样例数据")
    ...
    invalid = data[(data["lead_day"] < 0) | ~data["wind_speed"].between(0.0, 35.0, inclusive="both")]
    if not invalid.empty:
        raise ValueError(f"气象预报样例数据存在非法提前期或风速，非法行数={len(invalid)}")
...
def clean_monthly_truth(raw_truth: pd.DataFrame) -> pd.DataFrame:
    require_columns(raw_truth, ["month", "energy_true"], "月度真值样例数据")
    data = raw_truth.copy()
    ...
    if not invalid.empty:
        raise ValueError(f"月度真值必须为正数，非法行数={len(invalid)}")

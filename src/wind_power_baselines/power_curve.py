...
def assign_wind_speed_bin(wind_speed: pd.Series, bin_width: float) -> pd.Series:
    if bin_width <= 0.0:
        raise ValueError(f"wind speed bin width must be positive; got {bin_width}")
...
def build_power_curve(cleaned_scada: pd.DataFrame, bin_width: float, min_samples: int) -> pd.DataFrame:
    require_columns(cleaned_scada, ["turbine_id", "season", "wind_speed", "power_kw"], "cleaned SCADA data")
    if min_samples <= 0:
    ...
    grouped = data.groupby(["turbine_id", "season", "wind_speed_bin"], as_index=False).agg(power_kw=("power_kw", "median"), sample_count=("power_kw", "size")).sort_values(["turbine_id", "season", "wind_speed_bin"])
    curve = grouped[grouped["sample_count"] >= min_samples].copy()
    if curve.empty:
        raise ValueError(f"power curve is empty; lower min_samples or inspect SCADA coverage: {min_samples}")
    return curve.reset_index(drop=True)


def estimate_power_from_curve(wind_speed: pd.Series, season: pd.Series, power_curve: pd.DataFrame) -> pd.Series:
    require_columns(power_curve, ["season", "wind_speed_bin", "power_kw"], "power curve data")

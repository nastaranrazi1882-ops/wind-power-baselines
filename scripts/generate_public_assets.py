...
def create_baseline_notebook(project_root: Path) -> None:
    cells = [
        ...
        markdown_cell("## 6. Public Result Analysis\n\n下面读取真实工程脱敏后的结果。这里不展示真实 kWh/GWh，只展示定稿 W1_7个月窗口上的 MAPE、Bias、PearsonR，以及逐月归一化指数。W1 覆盖 2025-07 到 2026-01 共 7 个目标月，5 条方法使用同一组月份比较。"),
        ...
        code_cell("plot_monthly = public_monthly.copy()\nplot_monthly['month_date'] = pd.PeriodIndex(plot_monthly['month'].astype(str), freq='M').to_timestamp()\nplot_monthly = plot_monthly.sort_values(['month_date', 'method'])\nmonth_counts = plot_monthly.groupby('method')['month'].nunique()\nassert month_counts.nunique() == 1, month_counts.to_dict()\nfig, ax = plt.subplots(figsize=(11, 5))\nsns.lineplot(data=plot_monthly, x='month_date', y='energy_pred_index', hue='method', marker='o', ax=ax)\nsns.lineplot(data=plot_monthly.drop_duplicates('month').sort_values('month_date'), x='month_date', y='energy_true_index', color='black', marker='o', label='Truth index', ax=ax)\nax.tick_params(axis='x', rotation=45)\nax.set_title('Normalized monthly predictions and truth, W1 final window')\nax.set_xlabel('month')\nax.set_ylabel('Index, W1 truth mean = 1')\nfig.tight_layout()"),
        markdown_cell("## 7. Actual Public Figures And Conclusions\n\n下面几张图由真实评估结果和真实中间指标渲染，公开仓库只保留 PNG 与脱敏指标，不保留原始电量表、风塔表或本地路径。最终对比图统一使用定稿 W1_7个月窗口，避免把不同方法的可用月份混在一起。\n\n![Baseline MAPE and bias](../figures/point_forecast/actual_baseline_mape_bias.png)\n\n**结论。** W1 口径下，AK-D 的 MAPE 约 14.91%、PearsonR 约 0.8837，是 5 条公开主线里综合表现最好的方法之一。Baseline 1/2 作为历史参照较稳，MAPE 分别约 13.73% 和 17.86%，但两者都有明显负偏差。Baseline 3 的汇总 Bias 是 +0.38%，图上柱子很贴近零线；这不是说 Baseline 3 逐月误差小，而是 2025-08、2026-01 的高估抵消了 2025-11、2025-12 等月份的低估。因此 Baseline 3 的 MAPE 仍有约 23.93%，必须和逐月误差一起看。Baseline 4 的 MAPE 约 21.90%，相关系数约 0.6357，相似日路线在趋势上有一定信息，但异常小风月仍会高估。\n\n![Monthly prediction index](../figures/point_forecast/actual_monthly_prediction_index.png)\n\n**结论。** 这张图现在只画 W1 的 7 个目标月，5 条方法每个月都有点，所以拐点是一一对齐的。2025-08 是所有气象驱动方法的主要高估月，Baseline 3 和 Baseline 4 尤其明显；2025-11/12 真值进入高发阶段，多数方法低估，说明历史均值、功率曲线和分月订正都没有完全抓住冬季高发强度。AK-D 在 2025-07、2025-11、2025-12、2026-01 仍低估，但整体跟随月度起伏更好，因此相关系数最高。\n\n![Monthly relative error heatmap](../figures/point_forecast/actual_monthly_error_heatmap.png)\n\n**结论。** 热力图把 W1 各月误差展开后，可以看到 Baseline 3 的 MAPE 高主要来自 2025-08 的大幅高估和 2025-11 的低估；Baseline 4 的主要问题也是 2025-08 高估和 2025-11/12 低估。Baseline 1/2 虽然更简单，但误差分布更平滑。AK-D 没有完全消除 2025-08 高估，但相比 Baseline 3/4 明显压低了异常月误差，是分月订正有效的直接证据。\n\n![Lead-day error and correlation](../figures/point_forecast/actual_forecast_lead_day_error.png)\n\n**结论。** 提前期图只对 Baseline 3/4 展开，因为它们依赖 EC45 起报和提前期。Baseline 3 的误差随提前期增加整体抬升，相关性也下降，说明 LLS 订正对较近预报更有效；Baseline 4 的短提前期相关性较好，但误差水平不低，说明相似日筛选能抓到部分趋势，却容易在电量尺度上偏离。最终月度点预测要同时看提前期、月内覆盖和功率曲线映射，不能只看单日预报技巧。"),
